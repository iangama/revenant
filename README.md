...
The repository has completed and integrated **M23 — Client Architecture and Validation Refactor** on top of the published 0.2.0 baseline and is planning **M24 — Closed Playtest Readiness and Product Validation**. The playable `relay_awakening` slice retains its server-authoritative inventory, progression, loadout, combat, and activity flow while explicit client protocol, transport, projection, input, presentation, and validation boundaries reduce coordinator concentration without changing behavior or the published version.
