...
if [[ "$version" != "$canonical_version" ]]; then
  echo "release version $version does not match VERSION ($canonical_version)" >&2
  exit 1
fi
...
cp -R client/game/presentation "$release_dir/game/"
cp -R client/game/audio "$release_dir/game/"
